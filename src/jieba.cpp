...
struct FullSegment
{
    static void cut(const DartsDict & dict, const Runes & runes_data, size_t begin, size_t end, RuneRanges & ranges)
    {
        ...
        /// `fine_grained`: ASCII is handled uniformly by `segmentWithAsciiHandling` (same as
        /// `coarse_grained`), and each non-ASCII subrange is enumerated over the dictionary DAG
        /// to emit all overlapping word candidates.
        auto full_cut = [&](size_t sub_begin, size_t sub_end)
        {
        ...
        segmentWithAsciiHandling(runes, begin, end, ranges, full_cut);
    }
};
